...
export const Route = createFileRoute("/register")({
  head: () => ({ meta: [{ title: "Register — HASTON" }, { name: "description", content: "Create a HASTON account." }] }),
  component: Register,
});
    ...
    <div className="grid min-h-[85vh] grid-cols-1 md:grid-cols-2">
      <div className="grid place-items-center px-6 py-16 md:px-16 md:order-2">
        ...
        >
          ...
          <p className="mt-4 text-sm text-muted-foreground">Private previews, invitations and lifetime alterations.</p>
          ...
          <LuxeButton className="mt-8 w-full" arrow>Create account</LuxeButton>
...
function Input({ label, type = "text" }: { label: string; type?: string }) {
  return (
    <label className="block">
      ...
      <input type={type} className="mt-2 block w-full border-b border-border bg-transparent px-1 py-3 text-sm transition-colors focus:border-primary focus:outline-none" />
    </label>
  );
}
