...
export const Route = createFileRoute("/orders")({
  head: () => ({ meta: [{ title: "Orders — HASTON" }, { name: "description", content: "Your HASTON order history." }] }),
  component: Orders,
});
...
function Orders() {
  ...
  return (
    <>
      <PageHero eyebrow="Account" title="Your orders." breadcrumb={[{ label: "Account", to: "/account" }, { label: "Orders" }]} />
          ...
          {orders.map(o => (
            <div key={o.id} className="rounded-md border border-border bg-card p-6 soft-shadow">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {o.items.map(i => <img key={i.id} src={i.image} alt="" className="h-24 w-20 rounded object-cover" />)}
              </div>
            </div>
          ))}
        </div>
