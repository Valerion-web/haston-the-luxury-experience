...
function NotFound() {
  return (
    <section className="grid min-h-[80vh] place-items-center px-6 text-center">
      <div>
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-eyebrow text-muted-foreground">Error 404</motion.p>
        <motion.h1
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-4">
          <LuxeButton to="/" arrow>Return home</LuxeButton>
          <Link to="/collections" className="rounded-full border border-border px-8 py-4 text-[11px] uppercase tracking-[0.28em]">Browse collections</Link>
        </div>
      </div>
    </section>
  );
