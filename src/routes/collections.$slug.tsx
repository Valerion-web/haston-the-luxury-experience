...
export const Route = createFileRoute("/collections/$slug")({
  head: ({ params }) => ({
    meta: [
      ...
      { name: "description", content: `Shop the ${params.slug.replace(/-/g, " ")} collection from HASTON.` },
    ],
  }),
...
const SORTS = ["Featured", "New arrivals", "Price: Low to high", "Price: High to low", "Best rated"];
...
function CollectionPage() {
  ...
  const products = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "New arrivals": list = [...list].sort((a, b) => Number(!!b.isNew) - Number(!!a.isNew)); break;
    }
    // dedupe
      ...
      <section className="mx-auto max-w-[1600px] px-6 py-12 md:px-10">
        <div className="flex items-center justify-between gap-4 pb-8 hairline">
          ...
          <div className="relative">
            <button onClick={() => setSortOpen(v => !v)} className="inline-flex items-center gap-2 rounded-full border border-border px-5 py-2.5 text-[11px] uppercase tracking-[0.28em]">
            ...
            <AnimatePresence>
              {sortOpen && (
                ...
                >
                  {SORTS.map(s => (
                    <button key={s} onClick={() => { setSort(s); setSortOpen(false); }} className={`block w-full rounded px-4 py-2.5 text-left text-xs uppercase tracking-[0.24em] transition-colors hover:bg-muted ${sort === s ? "bg-muted" : ""}`}>
                      {s}
                    </button>
                  ))}
                </motion.div>
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[280px_1fr]">
          <AnimatePresence initial={false}>
            {open && (
              ...
              >
                <FilterPanel {...{ priceMax, setPriceMax, selColors, selSizes, toggle, setSelColors, setSelSizes }} />
              </motion.aside>
            )}
          </AnimatePresence>
          ...
          <div className={open ? "lg:col-start-2" : "lg:col-span-2"}>
            {products.length === 0 ? (
              <div className="grid place-items-center py-32 text-center">
                ...
                <button onClick={() => { setSelColors([]); setSelSizes([]); setPriceMax(400); }} className="mt-6 text-[11px] uppercase tracking-[0.28em] underline">
                  Reset filters
                </button>
              </div>
            ) : (
  ...
  return (
    <div className="space-y-10">
      <div>
        ...
        <input
          type="range" min={50} max={400} value={p.priceMax} onChange={(e) => p.setPriceMax(+e.target.value)}
          className="mt-6 w-full accent-primary"
        />
        <div className="mt-2 flex justify-between text-[11px] text-muted-foreground">
          <span>$50</span><span>Up to ${p.priceMax}</span>
        </div>
      </div>
      <div>
        ...
        <div className="mt-6 grid grid-cols-4 gap-3">
          {ALL_COLORS.map(c => (
            <button key={c.name} onClick={() => p.toggle(p.selColors, c.name, p.setSelColors)} aria-label={c.name}
      ...
      <div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {ALL_SIZES.map(s => (
            <button key={s} onClick={() => p.toggle(p.selSizes, s, p.setSelSizes)}
