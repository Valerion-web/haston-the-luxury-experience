...
export const Route = createFileRoute("/cart")({
  head: () => ({ meta: [{ title: "Cart — HASTON" }, { name: "description", content: "Your HASTON bag." }] }),
  component: Cart,
});
      ...
      <section className="mx-auto max-w-[1600px] px-6 py-16 md:px-10">
        {items.length === 0 ? (
          <div className="grid place-items-center py-24 text-center">
            ...
            <LuxeButton to="/collections" className="mt-8" arrow>Continue shopping</LuxeButton>
          </div>
        ) : (
            ...
            <div>
              ...
              <AnimatePresence initial={false}>
                {items.map(({ product, qty, size, color, id }) => (
                  ...
                  >
                    <Link to="/product/$slug" params={{ slug: product.slug }} className="block overflow-hidden rounded">
                      <img src={product.image} alt={product.name} className="aspect-[4/5] w-full object-cover" />
                    </Link>
                    <div className="min-w-0">
                      <Link to="/product/$slug" params={{ slug: product.slug }} className="text-display text-lg">{product.name}</Link>
                    ...
                    <div className="flex items-center gap-1 justify-self-center rounded-full border border-border p-1">
                      <button onClick={() => setLines(ls => ls.map(l => l.id === id ? { ...l, qty: Math.max(1, l.qty - 1) } : l))} className="grid h-8 w-8 place-items-center rounded-full hover:bg-muted"><Minus className="h-3 w-3" /></button>
                      <span className="w-7 text-center text-sm">{qty}</span>
                      <button onClick={() => setLines(ls => ls.map(l => l.id === id ? { ...l, qty: l.qty + 1 } : l))} className="grid h-8 w-8 place-items-center rounded-full hover:bg-muted"><Plus className="h-3 w-3" /></button>
                    </div>
            ...
            <aside className="md:sticky md:top-32 md:self-start">
              <div className="rounded-md border border-border bg-card p-8 soft-shadow">
                ...
                <div className="mt-6 space-y-3 text-sm">
                  ...
                  {applied && <div className="flex justify-between text-accent"><span>Discount (WELCOME10)</span><span>-${discount}</span></div>}
                ...
                <div className="mt-6 hairline pt-6">
                  <div className="flex justify-between text-lg"><span className="text-display">Total</span><span className="font-medium">${total}</span></div>
                </div>

                <form onSubmit={e => { e.preventDefault(); if (promo) setApplied(true); }} className="mt-6 flex items-center gap-2 rounded-full border border-border p-1.5">
                  <Tag className="ml-3 h-3.5 w-3.5 text-muted-foreground shrink-0" />
                  <input value={promo} onChange={e => setPromo(e.target.value)} placeholder="Promo code" className="min-w-0 flex-1 bg-transparent px-2 py-2 text-sm focus:outline-none" />
                  <button className="rounded-full bg-primary px-4 py-2 text-[10px] uppercase tracking-[0.28em] text-primary-foreground">Apply</button>
                </form>

                <LuxeButton to="/checkout" className="mt-6 w-full" arrow>Checkout securely</LuxeButton>
