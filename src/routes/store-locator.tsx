...
export const Route = createFileRoute("/store-locator")({
  head: () => ({ meta: [{ title: "Store Locator — HASTON" }, { name: "description", content: "Find a HASTON store worldwide." }] }),
...
function StoreLocator() {
  return (
    <>
      <PageHero eyebrow="Stores" title="Visit the house." description="Twelve ateliers, worldwide. Private appointments on request." breadcrumb={[{ label: "Stores" }]} />
          ...
          {STORES.map((s, i) => (
            ...
            >
              ...
              <div className="mt-6 space-y-3 text-sm text-muted-foreground">
                <p className="flex items-start gap-3"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-accent" strokeWidth={1.4}/> {s.address}</p>
                <p className="flex items-center gap-3"><Phone className="h-4 w-4 shrink-0 text-accent" strokeWidth={1.4}/> {s.phone}</p>
                <p className="flex items-center gap-3"><Clock className="h-4 w-4 shrink-0 text-accent" strokeWidth={1.4}/> {s.hours}</p>
              </div>
              <button className="mt-8 text-[11px] uppercase tracking-[0.28em] underline">Book an appointment</button>
            </motion.div>
          ))}
        </div>
      </section>
