...
const tiles = [
  { img: IMG.lb1, label: "Linen Hours", meta: "Editorial 01", span: "md:col-span-2 md:row-span-2" },
  { img: IMG.catShirts, label: "The Shirt Study", meta: "Fabric", span: "" },
  { img: IMG.catTrousers, label: "Office Ease", meta: "Tailoring", span: "" },
  { img: IMG.lb2, label: "Weekend Denim", meta: "Lifestyle", span: "md:col-span-2" },
  ...
  return (
    <section ref={ref} className="relative overflow-hidden texture-linen">
      <div className="mx-auto max-w-[1600px] px-5 py-10 md:px-8 md:py-14">
        ...
        <motion.div
          style={{ y }}
          className="mt-6 grid grid-cols-2 gap-2.5 md:grid-cols-4 md:gap-3"
        >
          {tiles.map((t, i) => (
            <motion.div
              key={t.label}
            ...
            >
              <Link to="/lookbook" className="block h-full">
                <div className={`h-full w-full ${t.span ? "aspect-square md:aspect-auto md:h-full" : "aspect-square"}`}>
                  <img
                    src={t.img}
                    alt={t.label}
                    loading="lazy"
                    decoding="async"
                    className="h-full min-h-[140px] w-full object-cover transition-transform duration-[1300ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-[1.07]"
                  />
                </div>
